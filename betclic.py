...
logger = logging.getLogger('betclic')
...
def extract_ng_state(html_content):
    ...
    try:
        ...
        logger.error(f"Błąd podczas parsowania ng-state: {e}")
    return {}
...
def fetch_page(url, attempts=1, retry_statuses=None):
    ...
    logger.error(f"Błąd pobierania {url}: {last_error}")
    return None
...
def collect_match_urls_from_competitions(competitions):
    ...
    for cid, competition_name in sorted(competitions.items(), key=lambda item: item[1].lower()):
        ...
        logger.info(f"Turniej: {competition_name} -> {len(links)} linków")
...
def fetch_additional_grouped_markets(match_id, category_id):
    ...
    try:
        ...
        logger.error(f"Nie udało się dociągnąć kategorii {category_id} dla match_id={match_id}: {e}")
        return []
...
with open(out_file_name, 'w') as outfile:
    ...
    logger.info(f"Pomijanie deblowych: {'TAK' if SKIP_DOUBLES else 'NIE'}")
    logger.info(f"Wybrane turnieje: {len(competitions)}")
    ...
    logger.info(f"Znaleziono {len(match_urls)} linków do meczy")
    ...
    for match_url in match_urls:
        ...
        if not match_details:
            logger.debug(f"Pomijam (live lub brak danych): {match_url}")
            continue
        ...
        try:
            ...
            for player in [player1, player2]:
                if player and player not in tennis_dictionary['players']:
                    # Raw, unprefixed print: scanned verbatim by suggest_dictionary_updates.py, do not route through logging.
            ...
            logger.error(f"Błąd dla {player1}\t{player2}\t{match_id}")
            logger.error(f"{e}")
            continue

        time.sleep(REQUEST_DELAY_SECONDS)

    logger.info(f"Przetworzono {matches_processed} meczy")
...
function filterTable() {{
  ...
  document.querySelectorAll('table.t tbody tr').forEach(r => {{
    ...
    with open(html_file, 'w', encoding='utf-8') as f:
        f.write(html_out)
    logger.info(f"HTML tabelka: {html_file} ({len(df)} wierszy)")
except Exception as e:
    logger.error(f"Nie udało się wygenerować HTML: {e}")
